...
### [x] #8 朝7時記事取得の失敗とゲスト/ログイン時の記事非表示
- **原因**: `fetched_at` はUTC保存。GitHub Actionsが22:xx UTCに実行するため、9:00 JST（= 0:00 UTC）を過ぎると `date.today()`（UTC）が翌日になり日付フィルタが外れる
- **修正**: 日付比較をJST基準に変更（`datetime.now(JST).date()` + `fetched_at + timedelta(hours=9)`）、cronを21:50 UTCに前倒し
