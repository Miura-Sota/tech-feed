...
### [ ] #8 朝7時記事取得の失敗とゲスト/ログイン時の記事非表示
- **背景**: 毎朝7時の記事取得が失敗しており、ゲストまたはログイン後に記事が見れない。GitHub Actions は成功しているが「Today at 7:22 AM」と表示されており、7時ちょうどに実行されていない可能性がある。
- **関連ファイル**: `.github/workflows/daily_fetch.yml`, `backend/routers/articles.py`, フロントの記事取得・表示
- **調査・検討**:
  - ワークフローの cron が 7:00 JST で発火しているか（UTC 0:00 等の設定ミスがないか）
  - 7:22 実行でも記事は取得されるはずなので、ゲスト/ログイン時の「見れない」原因（API 認証・日付フィルタ・空レスポンスの扱い）を別途調査
  - 必要なら cron を前倒し（例: 6:50 JST）にして 7 時前に完了させる
